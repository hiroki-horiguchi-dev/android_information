...
- その後、Kotlinにおける非同期プログラミングの実装であるKotlinコルーチンについて説明
  - Dispatcher 周りの話に関して詳しくなりたいんだよね
...
- その代わりに、関数は結果(と言うよりは Token )を囲むラッパーを返す
...
```
- そこで、suspend修飾子を追加してsubmitPost関数をサスペンド関数にしたのですが、なぜか非同期になってしまいました。なぜだろう？ --> なんだこの書き方は
...
```
- サスペンディング関数postItemは、コルーチンか他のサスペンディング関数からのみ呼び出されるべ
